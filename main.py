    ...
    if data_type == "Employees":
        ...
        for i, employee in enumerate(employees):
            with st.expander(f"ID: {employee['id']}, Name: {employee['name']}"):
                new_name = st.text_input(f"Edit Name for {employee['name']}", value=employee['name'])
                ...
                new_certifications = st.multiselect(
                    "Edit Certifications", certifications_list, default=valid_certifications, key=f"edit_cert_{i}"
                )
                
                valid_skills = [s for s in skills_list if s in employee['skills']]
                new_skills = st.multiselect(
                    "Edit Skills", skills_list, default=valid_skills, key=f"edit_skill_{i}"
                )
        ...
        certifications = st.multiselect('Certifications', certifications_list)
        skills = st.multiselect('Skills', skills_list)
        
        if st.button('Submit'):
            new_id = "W" + str(len(worker_vectors) + 1).zfill(3)
            new_employee = {
                "id": new_id,
            ...
            worker_vectors.append(new_employee)
            save_data(worker_vectors, './employee_data.json')  # Simpan data setelah menambahkan karyawan baru
        ...
        for i, certification in enumerate(certifications):
            with st.expander(certification):
                new_certification = st.text_input("Edit Certification", value=certification)
        ...
        for i, skill in enumerate(skills):
            with st.expander(skill):
                new_skill = st.text_input("Edit Skill", value=skill)
